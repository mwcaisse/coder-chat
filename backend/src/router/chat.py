...
def fetch_chat_r(chat_id: UUID, user=Depends(verify_auth_token)):
...
@router.post("/chat/")
def create_chat_r(user=Depends(verify_auth_token)):
    chat = create_chat()
...
def send_message(
    chat_id: UUID, model: ChatRequestModel, user=Depends(verify_auth_token)
):
